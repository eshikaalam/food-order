...
while($comment = mysqli_fetch_assoc($commentsResult)){
	$commentHTML .= '
		...
		<div class="panel-footer" align="right"><button type="button" class="btn btn-success reply" id="'.$comment["id"].'">Reply</button></div>
		</div> ';
...
function getCommentReply($conn, $parentId = 0, $marginLeft = 0) {
	...
	if($commentsCount > 0) {
		while($comment = mysqli_fetch_assoc($commentsResult)){  
			$commentHTML .= '
				<div class="panel panel-success" style="margin-left:'.$marginLeft.'px">
				<div class="panel-heading">By <b>'.$comment["sender"].'</b> on <i>'.$comment["date"].'</i></div>
				<div class="panel-body">'.$comment["comment"].'</div>
				<div class="panel-footer" align="right"><button type="button" class="btn btn-success reply" id="'.$comment["id"].'">Reply</button></div>
				</div>
